...
async function getStepsData(mode, user_id) {
  ...
  if (mode === 'day') {
    ...
    data = rows.map(r => r.steps);

  } else if (mode === 'week' || mode === 'month') {
    query = `
      SELECT DATE(log_date) as date, steps 
      FROM steps 
      WHERE ${
        mode === 'week'
    ...
    data = rows.map(r => r.total_steps !== null ? parseInt(r.total_steps, 10) : 0);
